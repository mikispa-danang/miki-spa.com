(() => {
  ...
  async function getStaffProfile() {
    // RLS exposes only the authenticated staff member's own row.
    const { data, error } = await client.from('miki_crm_staff').select('email,role,active').limit(1).maybeSingle();
  ...
  async function loadCRM() {
    ...
    $('#newCount').textContent = (leads || []).filter(x => x.status === 'new').length + (bookings || []).filter(x => x.status === 'requested').length;
    $('#visitedCount').textContent = (leads || []).filter(x => x.status === 'visited').length + (bookings || []).filter(x => x.status === 'completed').length;
  }

  function sourceLabel(r) {
    return r.utm_source || r.utm_campaign || r.referrer || 'direct';
  }

  function renderLeads(rows) {
    $('#leads').innerHTML = `<table><thead><tr><th>Thời gian</th><th>Khách</th><th>Liên hệ</th><th>Dịch vụ</th><th>Nguồn</th><th>Trạng thái</th></tr></thead><tbody>${rows.map(r => `<tr><td>${esc(fmt(r.created_at))}</td><td>${esc(r.name)}</td><td class="contact">${esc(r.contact)}</td><td>${esc(r.service || '')}</td><td><span class="muted">${esc(sourceLabel(r))}</span></td><td><select class="status" data-table="miki_leads" data-id="${esc(r.id)}"><option ${r.status==='new'?'selected':''}>new</option><option ${r.status==='contacted'?'selected':''}>contacted</option><option ${r.status==='qualified'?'selected':''}>qualified</option><option ${r.status==='booked'?'selected':''}>booked</option><option ${r.status==='visited'?'selected':''}>visited</option><option ${r.status==='review'?'selected':''}>review</option><option ${r.status==='lost'?'selected':''}>lost</option></select></td></tr>`).join('')}</tbody></table>`;
  }

  function renderBookings(rows) {
    $('#bookings').innerHTML = `<table><thead><tr><th>Tạo lúc</th><th>Khách</th><th>Điện thoại</th><th>Dịch vụ</th><th>Lịch mong muốn</th><th>Nguồn</th><th>Trạng thái</th></tr></thead><tbody>${rows.map(r => `<tr><td>${esc(fmt(r.created_at))}</td><td>${esc(r.name)}</td><td class="contact">${esc(r.phone)}</td><td>${esc(r.service || '')}</td><td>${esc(r.appointment_date || '')} ${esc(r.appointment_time || '')}</td><td><span class="muted">${esc(sourceLabel(r))}</span></td><td><select class="status" data-table="miki_bookings" data-id="${esc(r.id)}"><option value="requested" ${r.status==='requested'?'selected':''}>requested</option><option value="confirmed" ${r.status==='confirmed'?'selected':''}>confirmed</option><option value="completed" ${r.status==='completed'?'selected':''}>completed</option><option value="cancelled" ${r.status==='cancelled'?'selected':''}>cancelled</option><option value="no_show" ${r.status==='no_show'?'selected':''}>no_show</option></select></td></tr>`).join('')}</tbody></table>`;
  }
  ...
  loginForm?.addEventListener('submit', async e => {
    ...
    const redirect = new URL('crm.html', location.href).href;
  ...
  async function boot() {
    ...
    if (!staff) { denied.hidden = false; return; }
    denied.hidden = true;
    app.hidden = false;
    const role = $('#staffRole');
    if (role) role.textContent = `${staff.email || session.user.email || ''} · ${staff.role || 'staff'}`;
